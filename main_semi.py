...
for r in range(repeatition):
    ...
    deviation_rate = 0.8
    minor_class_index = [4, 5, 6]
    sample_num = 20
    pns = PNS(deviation_rate, minor_class_index, idx_info, sample_num)

    if args.imb_ratio > 1:
        pure_node = pns.get_pure_node(data)
        data_train_mask, idx_info = split_semi_dataset_with_pure_node(len(data.x), n_data, n_cls, class_num_list,pure_node, idx_info, data.x.device)

        # deviation_node = pns.get_deviation_node(data)
        # data_train_mask, idx_info = split_semi_dataset_with_pure_node(len(data.x), n_data, n_cls, class_num_list, deviation_node, idx_info, data.x.device)

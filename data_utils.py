...
def split_semi_dataset_with_pure_node(total_node, n_data, n_cls, class_num_list, pure_node, idx_info, device):
    new_idx_info = []
    ...
    for i in range(n_cls):
        if n_data[i] > class_num_list[i]:
            pure_node_index = torch.nonzero(pure_node[p], as_tuple=False).squeeze()

            cls_idx = torch.randperm(len(pure_node_index))
            cls_idx = pure_node_index[cls_idx]

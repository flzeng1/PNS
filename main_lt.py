...
from pnSample.PNS import get_pure_node, get_deviation_node
...
setting_log = "Dataset: {}, ratio: {}, net: {}, n_layer: {}, feat_dim: {}, ens: {}, pns: {}".format(
    args.dataset, str(args.imb_ratio), args.net, str(args.n_layer), str(args.feat_dim), str(args.ens), str(args.pns))
...
for r in range(repeatition):
    ...
    if args.dataset in ['Cora', 'CiteSeer', 'PubMed']:
        ...
        x0 = torch.sum(data_train_mask == True)
        ...
        x1 = torch.sum(data_train_mask == True)

    if args.dataset in ['Coauthor-CS', 'Amazon-Computers', 'Amazon-Photo']:
    ...
    if args.pns:
        ...
        minor_class_index = list(range(n_cls))

        pure_nodes = get_pure_node(deviation_rate, n_data, minor_class_index, idx_info, data)
        # pure_nodes = get_deviation_node(deviation_rate, n_data, minor_class_index, idx_info, data)
        for i in range(n_cls):
            n_data[i] = (pure_nodes[i] == True).sum().item()

        node_num = data_train_mask.shape[0]
        new_data_train_mask = torch.zeros(size=(node_num,), dtype=bool, device=device)
        for i in range(n_cls):
            cls_pure_node_index = pure_nodes[i].squeeze()
            new_cls_train_data_index = idx_info[i][cls_pure_node_index]
            new_data_train_mask[new_cls_train_data_index] = True
        data_train_mask = new_data_train_mask

        row, col = data.edge_index[0], data.edge_index[1]
        row_mask = data_train_mask[row]
        col_mask = data_train_mask[col]
        edge_mask = row_mask & col_mask



    x2 = torch.sum(data_train_mask == True)
...
print(f"Memory used for forward pass: {(after_allocated - before_allocated) / 1024 ** 2:.2f} MB")
print(f"Total reserved after forward pass: {after_reserved / 1024 ** 2:.2f} MB")
